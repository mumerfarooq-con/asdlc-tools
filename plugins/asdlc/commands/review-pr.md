---
description: Review a PR — single reviewer by default, or a 3-panelist council with --council
...
Parse arguments:
- A number or Bitbucket URL → **Bitbucket mode**. A branch name → **local mode** (second token = base branch, default `develop`).
...
1. Invoke `pr-reviewer` with the PR id/URL or branch + base.
2. Relay verdict, severity counts, report path.
3. Continue to "Handle verdict".

## Council mode (--council — for critical / high-risk PRs)
1. Invoke these THREE panelists IN PARALLEL, each with the same PR id/URL or branch + base:
...
- `REQUEST_CHANGES`: ask the user what to do —
  ...
  - post findings as comments on the Bitbucket PR — ONLY with explicit user approval, comment-posting only, never approve/decline/merge, or
...
- Never perform any Bitbucket write action (comment, approve, decline, merge) without the user approving that specific action here.
